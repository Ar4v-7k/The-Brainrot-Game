...
def remove_small_islands(surface):
    ...
    minimum = max(120, int(largest * 0.018))
    out = surface.copy()
...
def process(input_path, key, outdir, frames=4):
    pygame.init()
    ...
    for i, frame in enumerate(normalized):
        ...
        pygame.image.save(out, str(outdir / f"{key}_idle_{i}.png"))
    pygame.quit()
...
def main():
    ...
    process(Path(args.input), args.key, Path(args.outdir), args.frames)
